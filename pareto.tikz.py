...
df_sorted_by_frequency.reset_index(drop=True, inplace=True)
df_sorted_by_costs = df.sort_values(by='Gesamtkosten', ascending=False)
df_sorted_by_costs.reset_index(drop=True, inplace=True)

sum = (df_sorted_by_costs['Gesamtkosten'].sum())

df_sorted_by_relative_cost = df_sorted_by_costs.copy()
df_sorted_by_relative_cost['Gesamtkosten'] = (df_sorted_by_relative_cost['Gesamtkosten'] / sum) * 100

cumulative_sum = 0
cumulative = []
for i in range(len(df_sorted_by_relative_cost['Gesamtkosten'])):
	cumulative_sum += df_sorted_by_relative_cost['Gesamtkosten'][i]
	cumulative.append(cumulative_sum)

index = np.arange(len(df_sorted_by_costs['Fehlerart']))
